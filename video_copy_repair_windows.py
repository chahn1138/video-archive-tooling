...
class ToolSet:
    ...
    def report(self, use_colour: bool = True) -> List[str]:
        ...
        for tool in self.REQUIRED:
            ...
            hint = "" if p else c('  →  winget install "FFmpeg (Essentials Build)"', YELLOW, use_colour)
            lines.append(f"    {tag}  {tool:<16} {p or ''}{hint}")
        for tool in self.OPTIONAL:
            p    = self.paths[tool]
            tag  = c("✓", GREEN, use_colour) if p else c("○ optional", DIM, use_colour)
            hint = "" if p else c("  →  winget install MKVToolNix", DIM, use_colour)
            lines.append(f"    {tag}  {tool:<16} {p or ''}{hint}")
        return lines
...
class VideoRepairEngine:
    ...
    def run(self):
        ...
        if not TOOLS.check_required():
            ...
            self._p(self._c('  Install: winget install "FFmpeg (Essentials Build)"', CYAN))
            sys.exit(1)
