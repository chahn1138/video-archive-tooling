...
class VideoRepairEngine:
    ...
    def _attach_file_log(self):
        ...
        fh = logging.FileHandler(self._log_path, encoding="utf-8",
                                 mode="a")   # always append across restarts
    ...
    def _init_journal(self) -> int:
        """
        Create the RunJournal, load any previous journal for this src+dst pair,
        and return the count of already-certified entries.
        """
        if self.dry_run:
            return 0
    ...
    def _header(self):
        ...
        if self.resume:
            ...
            if self._journal and self._journal.entry_count:
                self._p(f"  Journal : {self._c(str(self._journal.entry_count) + ' certified entries loaded — will skip health-check for these', GREEN)}")
            elif self._journal:
                self._p(f"  Journal : {self._c('no previous journal found — all destinations will be health-checked', DIM)}")
    ...
    def run(self):
        ...
        for idx, path in enumerate(self.files, 1):
            ...
            if self.resume and not self.dry_run:
                ...
                # If this script previously certified this file as done, trust
                # its own work unconditionally — no health-check needed.
                journaled, entry = (self._journal.is_done(rel)
                                    if self._journal else (False, {}))
                if journaled:
                    result.status      = "done"
                    result.sha256      = entry.get("sha256", "")
                    result.skip_reason = f"journal certified ({entry.get('status','?')} on {entry.get('run','?')})"
                ...
                # ── Step 2: Health-check for un-journaled destination files ───
                # These might be pre-existing files, files from a different run,
                # or files from an interrupted run that has no journal entry.
                ...
                if verdict == self._DST_CLEAN:
                    ...
                    # Write to journal so next restart uses the fast-path
                    if self._journal:
                    ...
                    try:
                        ...
                        pass
                # _DST_MISSING → fall through silently
            ...
            # Record to journal immediately (crash-safe append)
            if self._journal and not self.dry_run:
                self._journal.record(result)
        ...
        # Consolidate journal: merge previous + this run into a single clean file
        if self._journal and not self.dry_run:
