...
PLATFORM: Linux (Ubuntu / Debian / Fedora / Arch and derivatives)
          Run from any terminal emulator.

=== Required tools – install ONE of these ways ===
  Option A – apt (Debian / Ubuntu):
    sudo apt install ffmpeg mkvtoolnix
  Option B – dnf (Fedora / RHEL):
    sudo dnf install ffmpeg mkvtoolnix
  Option C – pacman (Arch):
...
=== Linux-specific notes ===
  ...
  - NFS / CIFS mounts work transparently via pathlib.
  - Paths are case-sensitive; duplicate detection preserves case.
  - ANSI colour is enabled automatically when stdout is a TTY.
    Use --no-colour for plain text (e.g. when piping to a log file).

=== Input modes ===
  Directory   video_copy_repair_linux.py /mnt/videos/ /mnt/backup/
  Single file video_copy_repair_linux.py clip.mp4 /mnt/backup/
  Glob        video_copy_repair_linux.py "/mnt/videos/*.mkv" /mnt/backup/
  Multiple    video_copy_repair_linux.py a.mp4 b.mkv "*.avi" /mnt/backup/
  NFS mount   video_copy_repair_linux.py /mnt/nas/videos/ /mnt/backup/
  List file   video_copy_repair_linux.py --list files.txt /mnt/backup/
...
def find_tool(name: str) -> Optional[str]:
    for directory in LINUX_SEARCH_PATHS:
...
class ToolSet:
    ...
    def report(self, use_colour: bool = True) -> List[str]:
        lines = []
        lines.append(f"    {c('Linux', CYAN, use_colour)}  {platform.release()}  "
                     f"({platform.machine()})")
        for tool in self.REQUIRED:
            p    = self.paths[tool]
            tag  = c("✓", GREEN, use_colour) if p else c("✗ MISSING", RED, use_colour)
            hint = "" if p else c('  →  sudo apt install ffmpeg  # or: dnf install ffmpeg / pacman -S ffmpeg', YELLOW, use_colour)
            lines.append(f"    {tag}  {tool:<16} {p or ''}{hint}")
        for tool in self.OPTIONAL:
            p    = self.paths[tool]
            tag  = c("✓", GREEN, use_colour) if p else c("○ optional", DIM, use_colour)
            hint = "" if p else c("  →  sudo apt install mkvtoolnix  # or: dnf install mkvtoolnix / pacman -S mkvtoolnix", DIM, use_colour)
            lines.append(f"    {tag}  {tool:<16} {p or ''}{hint}")
        return lines
...
class HWAccel:
    ...
    def best(self) -> Optional[str]:
        ...
        if self.vaapi: return "h264_vaapi"
    ...
    def label(self) -> str:
        ...
        if self.vaapi: return "AMD/Intel VAAPI"
...
def repair_reencode(src: Path, dst: Path, use_hwaccel: bool = True,
    ...
    if encoder == "h264_nvenc":
        ...
        codec_args = ["-vaapi_device", "/dev/dri/renderD128",
                      "-vf", "format=nv12,hwupload",
                      "-c:v", "h264_vaapi", "-qp", "22"]
        method = "VAAPI HW H.264"
...
class VideoRepairEngine:
    ...
    def _header(self):
        ...
        self._p(self._c("  VIDEO COPY + REPAIR UTILITY  ·  Linux", BOLD))
    ...
    def run(self):
        ...
        if not TOOLS.check_required():
            ...
            self._p(self._c('  Install: sudo apt install ffmpeg   # or dnf/pacman', CYAN))
            sys.exit(1)
